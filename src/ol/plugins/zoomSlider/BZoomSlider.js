...
var getListeners = function getListeners(target, type) {
...
var listen = function listen(target, type, listener, optThis, optOnce) {
...
var listenOnce = function listenOnce(target, type, listener, optThis) {
    return listen(target, type, listener, optThis, true);
};

var unListen = function unListen(target, type, listener, optThis) {
    var listeners = getListeners(target, type);
    if (listeners) {
...
var unListenByKey = function unListenByKey(key) {
    if (key && key.target) {
...
var clear = function clear(object) {
...
var addListener = function addListener(element, type, fn, context) {
...
export default class BZoomSlider {
    constructor(params) {
        ...
        var translateContent = this.createElement('div', 'hmap-zoom-slider-translate-content' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, this.element);

        var silderContent = this.createElement('div', 'hmap-zoom-slider-content' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, this.element);

        var translateN = this.createElement('div', 'hmap-zoom-slider-button hmap-zoom-slider-translate-n' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, translateContent);
        translateN.setAttribute('title', '向上平移');
        listen(translateN, 'click', this.handletranslateClick_.bind(this, 'translateN'));
        var translateS = this.createElement('div', 'hmap-zoom-slider-button hmap-zoom-slider-translate-s' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, translateContent);
        translateS.setAttribute('title', '向下平移');
        listen(translateS, 'click', this.handletranslateClick_.bind(this, 'translateS'));
        var translateW = this.createElement('div', 'hmap-zoom-slider-button hmap-zoom-slider-translate-w' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, translateContent);
        translateW.setAttribute('title', '向左平移');
        listen(translateW, 'click', this.handletranslateClick_.bind(this, 'translateW'));
        var translateE = this.createElement('div', 'hmap-zoom-slider-button hmap-zoom-slider-translate-e' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, translateContent);
        translateE.setAttribute('title', '向右平移');
        listen(translateE, 'click', this.handletranslateClick_.bind(this, 'translateE'));
        var zoomIn = this.createElement('div', 'hmap-zoom-slider-zoom-in' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, silderContent);
        zoomIn.setAttribute('title', '放大');
        listen(zoomIn, 'click', this.handleZoomClick_.bind(this, 1));

        var zoomOut = this.createElement('div', 'hmap-zoom-slider-zoom-out' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, silderContent);
        zoomOut.setAttribute('title', '缩小');
        listen(zoomOut, 'click', this.handleZoomClick_.bind(this, -1));

        var slider = this.createElement('div', 'hmap-zoom-slider-zoom-slider' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, silderContent);
        this.sliderBackgroundTop = this.createElement('div', 'slider-background-top' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, slider);
        this.sliderBackgroundBottom = this.createElement('div', 'slider-background-bottom' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, slider);
        var sliderBackgroundMask = this.createElement('div', 'slider-background-mask' + ' ' + BASE_CLASS_NAME.CLASS_SELECTABLE, slider);
        ...
        listen(this.silderContent, 'pointerdown', this.handleDraggerStart_, this);
        listen(this.silderContent, 'pointermove', this.handleDraggerDrag_, this);
        listen(this.silderContent, 'pointerup', this.handleDraggerEnd_, this);
        listen(this.silderContent, 'click', this.handleContainerClick_, this);
        listen(this.sliderBar, 'click', function(event) {
            event.stopPropagation();
        });
        var render = this.options['render'] ? this.options['render'] : control.BZoomSlider.render;
        Control.call(this, {
    ...
    getElement(selector, root) {
        ...
        var dom = function() {
            var found = void 0;
            return _root.document && /^#([\w-]+)$/.test(selector) ? (found = _root.getElementById(RegExp.$1)) ? [found] : [] : Array.prototype.slice.call(/^\.([\w-]+)$/.test(selector) ? _root.getElementsByClassName(RegExp.$1) : /^[\w-]+$/.test(selector) ? _root.getElementsByTagName(selector) : _root.querySelectorAll(selector));
        }();
        return dom;
    };
    ...
    initSlider_() {
        ...
        var thumb = getElement('.slider-bar', container)[0];
        ...
        if (containerSize.width > containerSize.height) {
            this.direction_ = control.BZoomSlider.Direction_.HORIZONTAL;
            this.widthLimit_ = containerSize.width - thumbWidth;
        } else {
            this.direction_ = control.BZoomSlider.Direction_.VERTICAL;
    ...
    handleDraggerStart_(event) {
        if (!this.dragging_ && event.target === getElement('.slider-bar', this.silderContent)) {
    ...
    handleDraggerDrag_(event) {
        if (this.dragging_) {
            var element = getElement('.slider-bar', this.silderContent)[0];
    ...
    setThumbPosition_(res) {
        ...
        var thumb = getElement('.slider-bar', this.silderContent)[0];
        if (this.direction_ === control.BZoomSlider.Direction_.HORIZONTAL) {
